...
const INITIAL_FLEET: PoliceUnit[] = [
  ...
  },
  {
    ...
    telemetry: { speedKmH: 38, fuelPercentage: 75, isSirenActive: false, radioChannel: '156.450 MHz', dashcamStatus: 'ONLINE', headingDegrees: 270 },
  ...
  },
  {
    ...
    telemetry: { speedKmH: 45, fuelPercentage: 81, isSirenActive: false, radioChannel: '156.600 MHz', dashcamStatus: 'RECORDING', headingDegrees: 0 },
  ...
  },
  {
    ...
    telemetry: { speedKmH: 30, fuelPercentage: 73, isSirenActive: false, radioChannel: '156.800 MHz', dashcamStatus: 'ONLINE', headingDegrees: 90 },
  ...
  },
  {
    ...
    telemetry: { speedKmH: 25, fuelPercentage: 85, isSirenActive: false, radioChannel: '156.600 MHz', dashcamStatus: 'ONLINE', headingDegrees: 0 },
  ...
  },
  {
    ...
    telemetry: { speedKmH: 20, fuelPercentage: 80, isSirenActive: false, radioChannel: '156.800 MHz', dashcamStatus: 'ONLINE', headingDegrees: 270 },
    routeHistory: [[21.1434953, 79.0809881]],
    lastPingTimestamp: new Date().toISOString(),
  },
  {
    id: 'PU020',
    callSign: 'Unit PU020 - Kalamna Chowk',
    badgeNumber: 'NTP-PU020',
    unitType: 'Interceptor Vehicle',
    vehicleModel: 'Tata Safari Interceptor',
    licensePlate: 'MH-31-P-020',
    officersAssigned: ['Insp. Sunita Rao'],
    stationBase: 'Kalamna Sector Post',
    zone: 'EAST',
    availability: 'AVAILABLE',
    location: { latitude: 21.1684000, longitude: 79.1450000, nearestJunctionId: 10, nearestJunctionName: 'Kalamna Chowk' },
    telemetry: { speedKmH: 30, fuelPercentage: 85, isSirenActive: false, radioChannel: '156.450 MHz', dashcamStatus: 'ONLINE', headingDegrees: 180 },
    routeHistory: [[21.1684000, 79.1450000]],
    lastPingTimestamp: new Date().toISOString(),
  },
];
  ...
  const loadBackendData = useCallback(async () => {
    ...
    if (!pRes.error && pRes.units.length > 0) {
      const dbFleet: PoliceUnit[] = pRes.units.map((u) => {
        const existing = INITIAL_FLEET.find((f) => f.id === u.id);
        const derivedZone = (u.zone || u.zone_code || existing?.zone || 'CENTRAL') as any;
        return {
          id: u.id,
          callSign: u.name,
          badgeNumber: u.badgeNumber || 'NGP-P-101',
          unitType: (u.unitType as any) || 'PCR Van',
          vehicleModel: existing?.vehicleModel || 'Mahindra Scorpio-N Patrol',
          licensePlate: existing?.licensePlate || 'MH-31-P-9900',
          officersAssigned: existing?.officersAssigned || ['Inspector V. Sharma'],
          stationBase: existing?.stationBase || 'Sitabuldi Station',
          zone: derivedZone,
          availability: (u.status as any) || 'AVAILABLE',
          location: {
            latitude: u.latitude,
            longitude: u.longitude,
            nearestJunctionId: existing?.location.nearestJunctionId || 1,
            nearestJunctionName: existing?.location.nearestJunctionName || 'Sitabuldi Chowk',
          },
          ...
          routeHistory: existing?.routeHistory || [[u.latitude, u.longitude]],
